...
const exportarExcel = () => {
  ...
  const nombreArchivo = `productos_${obtenerFechaActual()}.xlsx`;
...
const exportarCSV = () => {
  ...
  const nombreArchivo = `productos_${obtenerFechaActual()}.csv`;
  XLSX.writeFile(libro, nombreArchivo, { bookType: 'csv' });
};

const exportarJSON = () => {
  const datos = productosFiltrados.map(({ id, created_at, ...rest }) => rest);
  const blob = new Blob([JSON.stringify(datos, null, 2)], { type: 'application/json' });

  const nombreArchivo = `productos_${obtenerFechaActual()}.json`;
...
const exportarPDF = () => {
  ...
  const datos = productosFiltrados.map(p => [p.nombre, `$${p.precio}`, p.stock]);
  ...
  const nombreArchivo = `productos_${obtenerFechaActual()}.pdf`;
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ marginBottom: '20px' }}>
          <button onClick={exportarExcel} style={botonEstilo('#28a745')}>Descargar Excel</button>
          <button onClick={exportarCSV} style={botonEstilo('#ffc107', 'black')}>Descargar CSV</button>
          <button onClick={exportarJSON} style={botonEstilo('#17a2b8')}>Descargar JSON</button>
          <button onClick={exportarPDF} style={botonEstilo('#dc3545')}>Descargar PDF</button>
        </div>
...
const botonEstilo = (colorFondo, colorTexto = 'white') => ({
  marginRight: '10px',
  marginBottom: '10px',
  padding: '8px 16px',
